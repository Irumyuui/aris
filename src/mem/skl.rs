...
pub struct SklIter<K: Ord, A: MemAllocator> {
    skl: Arc<SkipList<K, A>>,
    node: *const Node<K>,
}

impl<K, A> SklIter<K, A>
where
    K: Ord,
    A: MemAllocator,
{
    ...
    pub fn status(&mut self) -> DBResult<()> {
        Ok(())
    }
}
...
mod tests {
    ...
    use crate::mem::{allocator::DefaultAllocator, skl::SklIter};
